...
def getLabelsToPredict(cell,dirs):
    f_dirs = dirs.copy()
    result = []
    for i in range(len(f_dirs)):
        if(cell == f_dirs[i][0:3]):
            result.append(dirs[i])
    return result


def getPredictions(list,current_assay,aa,basepath):
    f_list = list.copy()
    s1 = f_list[0][3:6]
    sim_sum = aa.loc[current_assay,s1]
    array = np.load(basepath+list[0])*(aa.loc[current_assay,s1])
    for i in range(len(list)):
        if(i == 0):
            continue
        s2 = f_list[i][3:6]
        temp = aa.loc[current_assay,s2]
        array = array + (np.load(basepath+list[i])*aa.loc[current_assay,s2])
        sim_sum = sim_sum + temp
    return array/sim_sum


def computePrediction(validation,aa,new_dirs,basepath,out):
    ...
    for i in range(len(validation)):
        ...
        list = getLabelsToPredict(current_cell,new_dirs)
        np.save(os.path.join(out+"/",validation[i]),getPredictions(list,current_assay,aa,basepath))
...
def main():
    ...
    cells_simil = pd.read_csv('outAxA.csv', index_col=0)
